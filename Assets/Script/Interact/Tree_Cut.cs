...
public class Tree_Cut : Interactable
{
    ...
    public override void Interact(Character character)
    {
        while (dropCount > 0)
        {
            ...
            Vector3 dropPosition = new Vector2(transform.position.x, transform.position.y + this.bound.extents.y); //아이템이 나타날 좌표(바운드 적용이 안됨..)
            dropPosition.x += spread * UnityEngine.Random.value - spread / 2;   // 분산값만큼 x좌표를 랜덤으로 떨어뜨림
            dropPosition.y += spread * UnityEngine.Random.value - spread / 2;   // 분산값만큼 y좌표를 랜덤으로 떨어뜨림
